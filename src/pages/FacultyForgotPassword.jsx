...
import { Mail, ArrowRight, GraduationCap, ArrowLeft } from 'lucide-react';
...
export default function FacultyForgotPassword() {
  ...
  const handleSubmit = async (e) => {
    ...
    setIsLoading(true);
    try {
      const { data } = await api.post('/auth/faculty-forgot-password', { email });
      setSuccess(true);
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col justify-center py-12 sm:px-6 lg:px-8 relative overflow-hidden">
      ...
      <div className="sm:mx-auto sm:w-full sm:max-w-md relative z-10">
        ...
        <p className="mt-2 text-center text-sm text-slate-500">Recover your portal access</p>
      </div>
        ...
        <div className="bg-white py-8 px-4 shadow-xl sm:rounded-2xl sm:px-10 border border-slate-100">
          {!success ? (
            <form className="space-y-6" onSubmit={handleSubmit}>
              <div>
                <label className="block text-sm font-medium text-slate-700">Faculty Email</label>
              ...
              <button type="submit" disabled={isLoading} className="w-full flex justify-center py-3 px-4 border border-transparent rounded-xl shadow-lg text-sm font-medium text-white bg-gradient-to-r from-emerald-600 to-teal-500 hover:from-emerald-500 hover:to-teal-400 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-emerald-500 transition-all disabled:opacity-50">
          ...
          <div className="mt-6 text-center">
            <Link to="/faculty-login" className="text-sm font-medium text-emerald-600 hover:text-emerald-500 flex items-center justify-center">
