...
import { KeyRound, ArrowRight, Landmark, Eye, EyeOff } from 'lucide-react';
...
export default function StudentResetPassword() {
  ...
  const handleSubmit = async (e) => {
    ...
    setIsLoading(true);
    try {
      const { data } = await api.post(`/auth/student-reset-password/${token}`, { password });
      ...
      <div className="mt-8 sm:mx-auto sm:w-full sm:max-w-md relative z-10">
        <div className="bg-white py-8 px-4 shadow-xl sm:rounded-2xl sm:px-10 border border-slate-100">
          ...
          <div className="mt-6 text-center">
            <Link to="/student-login" className="text-sm font-medium text-indigo-600 hover:text-indigo-500">Cancel & Return to Login</Link>
          </div>
        </div>
      </div>
