...
export default function AdminSignup() {
  ...
  const handleSignupSubmit = async (e) => {
    ...
    if (!name.trim() || !email.trim() || !password || !confirmPassword || !collegeName.trim() || !location.trim()) {
    ...
    if (!isValid) {
      triggerToast('error', 'Please resolve form errors before signing up.');
      return;
    }

    setLoading(true);
    const result = await register({ name, email, password, role: selectedRole, collegeName, location });
    setLoading(false);

    if (result.success) {
      triggerToast('success', `Account created successfully. Welcome, ${result.user.name}!`);
      navigate('/dashboard/admin');
    } else {
      triggerToast('error', result.message || 'Registration failed. Please try again.');
    }
  };
        ...
        <div className="hidden lg:flex lg:w-1/2 bg-slate-900/10 dark:bg-slate-950/30 backdrop-blur-[1px] border-r border-slate-200/50 dark:border-slate-800/40 relative flex-col justify-between p-12">
          <div className="flex items-center gap-3 relative z-10">
            ...
            <div>
              ...
              <p className="text-[10px] uppercase font-bold tracking-wider text-slate-400 dark:text-slate-500">Admin Portal</p>
            </div>
          </div>
            ...
            <AnimatePresence mode="wait">
              <motion.div initial={{ opacity: 0, x: -30 }} animate={{ opacity: 1, x: 0 }} className="space-y-6">
                ...
                <p className="text-slate-500 dark:text-slate-400 leading-relaxed text-sm md:text-base font-medium">
                  Execute direct server logs monitoring, security policies config, administrative directory modifications, database clusters state tracking, and performance metric gauges.
                </p>

                <div className="flex flex-wrap gap-2 pt-4">
                  <span className="px-3.5 py-1.5 rounded-full text-xs font-bold bg-rose-500/10 text-rose-600 dark:text-rose-400 border border-rose-500/10">Database Cluster Control</span>
                  <span className="px-3.5 py-1.5 rounded-full text-xs font-bold bg-pink-500/10 text-pink-600 dark:text-pink-400 border border-pink-500/10">Live Web Syslogs</span>
                </div>
              </motion.div>
            </AnimatePresence>
          </div>
          ...
          <div className="my-auto max-w-md w-full mx-auto space-y-8 pt-8">
            ...
            <div className="p-3.5 rounded-2xl border border-slate-200/60 dark:border-slate-800/80 bg-slate-50/50 dark:bg-slate-900/30 flex items-start gap-3">
              ...
              <div className="text-[11px] text-slate-500 dark:text-slate-400 leading-relaxed font-semibold">
                <span>ADMIN signup verification requires organization accounts ending with <strong className="text-rose-500">@admin.com</strong></span>
              </div>
            </div>

            <form onSubmit={handleSignupSubmit} className="space-y-4">
              <div>
                <label className="block text-[11px] font-bold text-slate-400 uppercase tracking-widest mb-2 font-mono">Full Name</label>
                <div className="relative">
                  <input type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g. Alex Johnson" className={`w-full px-4 py-3.5 rounded-2xl bg-slate-100/60 dark:bg-slate-900/60 border border-slate-200/80 dark:border-slate-800/80 focus:outline-none focus:ring-2 focus:ring-opacity-35 transition-all text-sm font-semibold ${theme.borderFocus}`} />
                </div>
              </div>

              <div>
                <label className="block text-[11px] font-bold text-slate-400 uppercase tracking-widest mb-2 font-mono">Organization Name</label>
                <div className="relative">
                  <input type="text" value={collegeName} onChange={(e) => setCollegeName(e.target.value)} placeholder="e.g. MuTeX Organization" className={`w-full px-4 py-3.5 rounded-2xl bg-slate-100/60 dark:bg-slate-900/60 border border-slate-200/80 dark:border-slate-800/80 focus:outline-none focus:ring-2 focus:ring-opacity-35 transition-all text-sm font-semibold ${theme.borderFocus}`} />
                </div>
              </div>

              <div>
                <label className="block text-[11px] font-bold text-slate-400 uppercase tracking-widest mb-2 font-mono">Location</label>
                <div className="relative">
                  <input type="text" value={location} onChange={(e) => setLocation(e.target.value)} placeholder="e.g. New York, United States" className={`w-full px-4 py-3.5 rounded-2xl bg-slate-100/60 dark:bg-slate-900/60 border border-slate-200/80 dark:border-slate-800/80 focus:outline-none focus:ring-2 focus:ring-opacity-35 transition-all text-sm font-semibold ${theme.borderFocus}`} />
                </div>
              </div>
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input type="text" value={email} onChange={handleEmailChange} placeholder="yourname@admin.com" className={`w-full pl-12 pr-4 py-3.5 rounded-2xl bg-slate-100/60 dark:bg-slate-900/60 border border-slate-200/80 dark:border-slate-800/80 focus:outline-none focus:ring-2 focus:ring-opacity-35 transition-all text-sm font-semibold ${theme.borderFocus}`} />
                </div>
                <AnimatePresence>
                  {emailError && (
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-[11px] font-bold text-slate-400 uppercase tracking-widest font-mono mb-2">Security Password</label>
                  <div className="relative">
                    <KeyRound className="absolute left-4 top-1/2 -translate-y-1/2 w-4.5 h-4.5 text-slate-400" />
                    <input type={showPassword ? 'text' : 'password'} value={password} onChange={(e) => setPassword(e.target.value)} placeholder="••••••••" className={`w-full pl-10 pr-10 py-3.5 rounded-2xl bg-slate-100/60 dark:bg-slate-900/60 border border-slate-200/80 dark:border-slate-800/80 focus:outline-none focus:ring-2 focus:ring-opacity-35 transition-all text-sm font-semibold ${theme.borderFocus}`} />
                    <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 transition-colors p-1 cursor-pointer">
                ...
                <div>
                  <label className="block text-[11px] font-bold text-slate-400 uppercase tracking-widest font-mono mb-2">Confirm Password</label>
                  <div className="relative">
                    <KeyRound className="absolute left-4 top-1/2 -translate-y-1/2 w-4.5 h-4.5 text-slate-400" />
              ...
              <div className="pt-2">
                <button type="submit" disabled={loading} className={`w-full flex items-center justify-center gap-2 py-3.5 rounded-2xl text-white font-bold text-sm shadow-xl transition-all cursor-pointer hover:brightness-105 active:scale-[0.98] disabled:opacity-75 disabled:pointer-events-none ${theme.primaryBtn}`}>
                  {loading ? (
                    ...
                    <>Complete Registration <ArrowRight className="w-4.5 h-4.5" /></>
                  )}
                </button>
              </div>
            </form>
            ...
            <div className="text-center pt-2">
              <p className="text-sm font-semibold text-slate-500 dark:text-slate-400">
                ...
                <Link to="/admin-login" className={`hover:underline cursor-pointer ${theme.textAccent}`}>
                  Sign in instead
                </Link>
              </p>
            </div>
            ...
            <div className="text-center pt-2">
              <Link to="/student-signup" className="text-sm font-bold text-slate-500 dark:text-slate-400 hover:text-rose-600 dark:hover:text-rose-400 flex items-center justify-center gap-1.5 transition-colors">
                <ArrowLeft className="w-4 h-4" /> Back to Student Signup
              </Link>
            </div>
